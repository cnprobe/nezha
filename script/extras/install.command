...
pre_check() {
    ...
    if [[ -n "${CUSTOM_MIRROR}" ]]; then
        GITHUB_RAW_URL="gitee.com/cnprobe/nezha/raw/V0"
...
install_agent() {
    ...
    local version=$(curl -m 10 -sL "https://api.github.com/repos/abcodingx/agent/releases/latest" | grep "tag_name" | head -n 1 | awk -F ":" '{print $2}' | sed 's/\"//g;s/,//g;s/ //g')
    if [ ! -n "$version" ]; then
        version=$(curl -m 10 -sL "https://gitee.com/api/v5/repos/abcodingx/agent/releases/latest" | awk -F '"' '{for(i=1;i<=NF;i++){if($i=="tag_name"){print $(i+2)}}}')
    fi
    if [ ! -n "$version" ]; then
        version=$(curl -m 10 -sL "https://fastly.jsdelivr.net/gh/abcodingx/agent/" | grep "option\.value" | awk -F "'" '{print $2}' | sed 's/abcodingx\/agent@/v/g')
    fi
    if [ ! -n "$version" ]; then
        version=$(curl -m 10 -sL "https://gcore.jsdelivr.net/gh/abcodingx/agent/" | grep "option\.value" | awk -F "'" '{print $2}' | sed 's/abcodingx\/agent@/v/g')
    fi

    if [ ! -n "$version" ]; then
        echo -e "Fail to obtaine agent version, please check if the network can link https://api.github.com/repos/abcodingx/agent/releases/latest"
        return 0
    else
    ...
    if [[ -z $CN ]]; then
        NZ_AGENT_URL="https://${GITHUB_URL}/abcodingx/agent/releases/download/${version}/nezha-agent_darwin_${os_arch}.zip"
    else
        NZ_AGENT_URL="https://${GITHUB_URL}/abcodingx/agent/releases/download/${version}/nezha-agent_darwin_${os_arch}.zip"
    fi
