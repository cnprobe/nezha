...
pre_check() {
    ...
    if [ -n "$CUSTOM_MIRROR" ]; then
        ...
        if [ -z "$CN" ]; then
            ...
            Get_Docker_URL="get.docker.com"
            Get_Docker_Argu=" "
            Docker_IMG="ghcr.io\/cnprobe\/nezha-dashboard:${NZ_DASHBOARD_VERSION}"
        else
            GITHUB_RAW_URL="gitee.com/cnprobe/nezha/raw/v0"
...
get_dashboard_version() {
    # 允许手动指定：NZ_DASHBOARD_VERSION=v0.21.1 ./nezha.sh ...
    if [ -n "$NZ_DASHBOARD_VERSION" ]; then
        return 0
    fi

    # 尝试 GitHub -> Gitee 兜底
    NZ_DASHBOARD_VERSION="$(curl -m 10 -sL "https://api.github.com/repos/cnprobe/nezha/releases/latest" \
        | awk -F'"' '/"tag_name"/{print $4; exit}')"

    if [ -z "$NZ_DASHBOARD_VERSION" ]; then
        NZ_DASHBOARD_VERSION="$(curl -m 10 -sL "https://gitee.com/api/v5/repos/cnprobe/nezha/releases/latest" \
            | awk -F'"' '/"tag_name"/{print $4; exit}')"
    fi

    if [ -z "$NZ_DASHBOARD_VERSION" ]; then
        err "获取 Dashboard 最新版本失败（cnprobe/nezha releases/latest）"
        return 1
    fi
}
...
install_agent() {
    ...
    _version=$(curl -m 10 -sL "https://api.github.com/repos/abcodingx/agent/releases/latest" | grep "tag_name" | head -n 1 | awk -F ":" '{print $2}' | sed 's/\"//g;s/,//g;s/ //g')
    if [ -z "$_version" ]; then
        _version=$(curl -m 10 -sL "https://gitee.com/api/v5/repos/abcodingx/agent/releases/latest" | awk -F '"' '{for(i=1;i<=NF;i++){if($i=="tag_name"){print $(i+2)}}}')
    fi
    if [ -z "$_version" ]; then
        _version=$(curl -m 10 -sL "https://fastly.jsdelivr.net/gh/abcodingx/agent/" | grep "option\.value" | awk -F "'" '{print $2}' | sed 's/abcodingx\/agent@/v/g')
    fi
    if [ -z "$_version" ]; then
        _version=$(curl -m 10 -sL "https://gcore.jsdelivr.net/gh/abcodingx/agent/" | grep "option\.value" | awk -F "'" '{print $2}' | sed 's/abcodingx\/agent@/v/g')
    fi

    if [ -z "$_version" ]; then
        err "获取 Agent 版本号失败, 请检查本机能否链接 https://api.github.com/repos/abcodingx/agent/releases/latest"
        return 1
    else
    ...
    echo "正在下载监控端"
    if [ -z "$CN" ]; then
        NZ_AGENT_URL="https://${GITHUB_URL}/abcodingx/agent/releases/download/${_version}/nezha-agent_linux_${os_arch}.zip"
    else
        NZ_AGENT_URL="https://${GITHUB_URL}/abcodingx/agent/releases/download/${_version}/nezha-agent_linux_${os_arch}.zip"
    fi
...
update_docker_compose_image() {
    ...
    get_dashboard_version || return 1
    ...
    if grep -q "ghcr.io/cnprobe/nezha-dashboard$" "$yaml_file_path"; then
        sed -i "s|ghcr.io/cnprobe/nezha-dashboard$|ghcr.io/cnprobe/nezha-dashboard:${NZ_DASHBOARD_VERSION}|" "$yaml_file_path"
    fi
}
...
restart_and_update_standalone() {
    ...
    if [ -z "$_version" ]; then
        _version=$(curl -m 10 -sL "https://fastly.jsdelivr.net/gh/cnprobe/nezha/" | grep "option\.value" | awk -F "'" '{print $2}' | sed 's/cnprobe\/nezha@/v/g')
    fi
    if [ -z "$_version" ]; then
        _version=$(curl -m 10 -sL "https://gcore.jsdelivr.net/gh/cnprobe/nezha/" | grep "option\.value" | awk -F "'" '{print $2}' | sed 's/cnprobe\/nezha@/v/g')
    fi
    if [ -z "$_version" ]; then
        _version=$(curl -m 10 -sL "https://gitee.com/api/v5/repos/cnprobe/nezha/releases/latest" | awk -F '"' '{for(i=1;i<=NF;i++){if($i=="tag_name"){print $(i+2)}}}')
    fi

    if [ -z "$_version" ]; then
        err "获取 Dashboard 版本号失败, 请检查本机能否链接 https://api.github.com/repos/cnprobe/nezha/releases/latest"
        return 1
    else
        echo "当前最新版本为： ${_version}"
    fi
    ...
    if [ -z "$CN" ]; then
        NZ_DASHBOARD_URL="https://${GITHUB_URL}/cnprobe/nezha/releases/download/${_version}/dashboard-linux-${os_arch}.zip"
    else
        NZ_DASHBOARD_URL="https://${GITHUB_URL}/cnprobe/nezha/releases/download/${_version}/dashboard-linux-${os_arch}.zip"
    fi
...
show_menu() {
    ...
    if get_dashboard_version >/dev/null 2>&1; then
        _dash_ver="$NZ_DASHBOARD_VERSION"
    else
        _dash_ver="unknown"
    fi
    ...
    ${yellow}Dashboard 版本: ${_dash_ver}${plain}
    --- https://github.com/cnprobe/nezha ---
    ${red}v0面板已停止维护, 请及时升级至v1, 再次致敬原作者们, 详见https://nezha.wiki/${plain}
    ${green}1.${plain}  安装面板端
